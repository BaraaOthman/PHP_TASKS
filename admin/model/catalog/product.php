...
$sql = "SELECT p.*, pd.*, (p.price - p.cost) AS profit FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
    ...
    // NEW: filter_cost with inequalities
    if (!empty($data['filter_cost'])) {
        $filter_cost = trim($data['filter_cost']);

        
        if (preg_match('/^(<=|>=|!=|<|>)\s*([0-9]+(\.[0-9]+)?)$/', $filter_cost, $m)) {
            $op  = $m[1];
            $val = (float)$m[2];
            $sql .= " AND p.cost " . $op . " '" . $this->db->escape($val) . "'";
       
        } elseif (preg_match('/^([0-9]+(\.[0-9]+)?)$/', $filter_cost, $m)) {
            $val = (float)$m[1];
            $sql .= " AND p.cost = '" . $this->db->escape($val) . "'";
        }
    }

	// NEW: filter_profit with inequalities on (price - cost)
if (!empty($data['filter_profit'])) {
    $filter_profit = trim($data['filter_profit']);

    if (preg_match('/^(<=|>=|!=|<|>)\s*([0-9]+(\.[0-9]+)?)$/', $filter_profit, $m)) {
        $op  = $m[1];
        $val = (float)$m[2];
        $sql .= " AND (p.price - p.cost) " . $op . " '" . $this->db->escape($val) . "'";
    } elseif (preg_match('/^([0-9]+(\.[0-9]+)?)$/', $filter_profit, $m)) {
        $val = (float)$m[1];
        $sql .= " AND (p.price - p.cost) = '" . $this->db->escape($val) . "'";
    }
}
...
public function getTotalProducts($data = array()) {
    ...
    // NEW: filter_cost with inequalities (must match getProducts)
    if (!empty($data['filter_cost'])) {
        $filter_cost = trim($data['filter_cost']);
